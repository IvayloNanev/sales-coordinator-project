...
export default function ValidationResults({ fileCount, totalRecords, results, onContinue, onReturn }) {
  ...
  return (
    <section className="screen" aria-labelledby="validation-title">
      <div className="screen-heading"><p className="eyebrow">Step 2 of 4</p><h1 id="validation-title">Review data validation</h1><p>We checked every row before including it in your report.</p></div>
      ...
      ) : <div className="alert success large"><strong>All records passed validation</strong><span>Your dataset is ready for approval.</span></div>}
      <div className="actions no-print"><button className="button secondary" onClick={onReturn}>← Return to Upload</button><button className="button primary" disabled={!results.validRecords.length} onClick={onContinue}>{issueCount ? "Exclude Invalid Records and Continue" : "Continue to Approval"} <span aria-hidden="true">→</span></button></div>
    </section>
  );
}
