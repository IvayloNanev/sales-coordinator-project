...
const numericValue = (value) => {
  ...
  return Number.isFinite(number) ? (percent ? number / 100 : number) : Number.NaN;
};
const cleanText = (value) => cleanCell(value).replace(/\s+/g, " ");
const comparisonKey = (value) => cleanText(value).toLocaleLowerCase();
const titleCase = (value) => cleanText(value).toLocaleLowerCase().replace(/\b\p{L}/gu, (letter) => letter.toLocaleUpperCase());
const SALES_SCOPE_HEADERS = new Set(["orderid", "orderdate", "salesrep", "unitssold", "orderstatus"]);
const isSalesScopeSheet = (headers) => [...SALES_SCOPE_HEADERS].every((header) => headers.includes(header));
...
function parseTableRows(rows, sourceFile = "Uploaded file", syntaxErrors = []) {
  ...
  const salesScopeSheet = isSalesScopeSheet(headers);
  ...
  const validFormat = (header, value) => {
    ...
    if (["quantity", "quantitysold", "unitssold"].includes(header)) return Number.isInteger(numericValue(value)) && numericValue(value) > 0;
    if (["sales", "revenue"].includes(header)) return Number.isFinite(numericValue(value));
    if (["discount", "discountpct"].includes(header)) return Number.isFinite(numericValue(value)) && numericValue(value) >= 0 && numericValue(value) <= 1;
  ...
  if (missingColumns.length || syntaxErrors.length) return { records: [], fileErrors, columnCount: headers.length, columnNames, previewRows, columnProfiles, isCandidateSheet: availableFields.size >= 3 || salesScopeSheet };

  const records = [];
  const seenSalesScopeOrders = new Set();
  rows.slice(1).forEach((values, index) => {
    ...
    if (salesScopeSheet) {
      ...
      if (seenSalesScopeOrders.has(orderKey)) return;
      seenSalesScopeOrders.add(orderKey);
      record.storeId = cleanText(record.salesRep) || "Unassigned sales rep";
      record.storeName = cleanText(record.salesRep) || "Unassigned sales rep";
      record.salesRegion = cleanText(record.salesRegion) || "Unassigned";
      record.customerName = cleanText(record.customerName) || `Customer ${cleanText(record.customerId) || "Unassigned"}`;
      record.productCategory = titleCase(record.productCategory);
      record.orderStatus = titleCase(record.orderStatus);
      if (numericValue(record.revenue) < 0) record.orderStatus = "Returned";
      record.lineItemId = `${sourceFile}:${index + 2}`;
      record.sourceProfile = "SalesScope";
    }
  ...
  return { records, fileErrors, columnCount: headers.length, columnNames, previewRows, columnProfiles, isCandidateSheet: true };
}
...
async function parseWorkbookFile(file) {
  try {
    ...
    const dataSheets = parsedSheets.filter((sheet) => sheet.isCandidateSheet);
    const selectedSheets = dataSheets.length ? dataSheets : parsedSheets;
    return {
      records: selectedSheets.flatMap((sheet) => sheet.records),
      fileErrors: selectedSheets.flatMap((sheet) => sheet.fileErrors),
      columnCount: Math.max(0, ...selectedSheets.map((sheet) => sheet.columnCount ?? 0)),
      columnNames: selectedSheets.find((sheet) => sheet.columnNames?.length)?.columnNames ?? [],
      previewRows: selectedSheets.find((sheet) => sheet.previewRows?.length)?.previewRows ?? [],
      columnProfiles: selectedSheets.find((sheet) => sheet.columnProfiles?.length)?.columnProfiles ?? [],
    };
  } catch (error) {
...
export function validateRecords(records, fileErrors = []) {
  ...
  records.forEach((record) => {
    ...
    const cleanedRecord = {
      ...
      orderStatus: titleCase(record.orderStatus),
    };
    ...
    const quantity = numericValue(record.quantitySold);
    const revenue = numericValue(record.revenue);
    if (cleanCell(record.quantitySold) && (!Number.isFinite(quantity) || quantity <= 0)) errors.push("Quantity must be a positive number");
    if (cleanCell(record.revenue) && (!Number.isFinite(revenue) || (revenue < 0 && comparisonKey(cleanedRecord.orderStatus) !== "returned"))) errors.push("Revenue must be nonnegative unless the order was returned");
    ...
    const recordIdentity = normalizedLineItem ? `line:${comparisonKey(cleanedRecord.sourceFile)}:${normalizedLineItem}` : `order:${normalizedOrder}`;
    ...
    if (errors.length) {
      ...
      validRecords.push({ ...cleanedRecord, quantitySold: quantity, revenue });
    }
  });
