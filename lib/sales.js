...
export function validateRecords(records, fileErrors = []) {
  ...
  records.forEach((sourceRecord) => {
    ...
    const recordIdentity = normalizedLineItem
      ...
      : `row:${[
        normalizedOrder,
        cleanCell(record.date).toLowerCase(),
        cleanCell(record.customerName).toLowerCase(),
        cleanCell(record.product).toLowerCase(),
        cleanCell(record.quantitySold).toLowerCase(),
        cleanCell(record.revenue).toLowerCase(),
      ].join("|")}`;
