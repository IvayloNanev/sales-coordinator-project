...
function parseTableRows(rows, sourceFile = "Uploaded file", syntaxErrors = []) {
  ...
  const validFormat = (header, value) => {
    ...
    if (header === "discount") return Number.isFinite(numericValue(value)) && numericValue(value) >= 0 && numericValue(value) <= 1;
...
async function parseWorkbookFile(file) {
  try {
    ...
    return {
      ...
      columnCount: Math.max(0, ...parsedSheets.map((sheet) => sheet.columnCount ?? 0)),
      columnNames: parsedSheets.find((sheet) => sheet.columnNames?.length)?.columnNames ?? [],
      previewRows: parsedSheets.find((sheet) => sheet.previewRows?.length)?.previewRows ?? [],
      columnProfiles: parsedSheets.find((sheet) => sheet.columnProfiles?.length)?.columnProfiles ?? [],
    };
  } catch (error) {
...
export function validateRecords(records, fileErrors = []) {
  ...
  records.forEach((record) => {
    ...
    if (!cleanedRecord.storeId) errors.push("Missing store ID");
    if (!cleanedRecord.storeName) errors.push("Missing store name");
    ...
    if (!cleanedRecord.productCategory) errors.push("Missing product category");
