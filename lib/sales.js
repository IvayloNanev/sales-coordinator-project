...
export function validateRecords(records, fileErrors = []) {
  ...
  records.forEach((sourceRecord) => {
    ...
    if (cleanCell(record.revenue) && !Number.isFinite(revenue)) errors.push("Revenue must be a number");
    ...
    const recordIdentity = normalizedLineItem
      ? `line:${comparisonKey(cleanedRecord.sourceFile)}:${normalizedLineItem}`
      : `row:${[
        normalizedOrder,
...
function aggregateOrders(records) {
  ...
  records.forEach((record) => {
    const orderKey = comparisonKey(record.orderNumber);
  ...
  return [...orders.values()]
    .map(({ products, discountTotal, lineItems, ...order }) => ({
      ...order,
      products: [...products.values()].join(", "),
      averageDiscount: lineItems ? discountTotal / lineItems : 0,
      profitMargin: order.revenue ? (order.profit / order.revenue) * 100 : 0,
    }))
    .sort((a, b) => b.revenue - a.revenue);
}
...
export function calculateReport(records) {
  ...
  const uniqueOrders = new Set(records.map((record) => comparisonKey(record.orderNumber))).size;
...
export class SalesDataManager {
  constructor(records = []) {
    this.records = records.map((record) => ({
      ...
      discount: Number.isFinite(percentageValue(record.discount)) ? percentageValue(record.discount) : 0,
...
export function generateSummary(report, startDate, endDate, context = {}) {
  ...
  const movement = changes?.revenue?.percentage === null
    ...
    : changes?.revenue
      ...
      : null;
  const profitMovement = changes?.profit?.percentage === null
    ? null
    : changes?.profit
  ...
  const performance = ` ${report.highestRevenueRegion} led regional sales and ${report.highestRevenueCategory} was the strongest category. Profit was ${formatCurrency(report.totalProfit)} at a ${report.profitMargin.toFixed(1)}% margin${profitMovement ? `, ${profitMovement} versus the prior period` : ""}.`;
