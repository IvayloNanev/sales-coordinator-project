...
function parseTableRows(rows, sourceFile = "Uploaded file", syntaxErrors = []) {
  if (!rows.length) {
    return { records: [], fileErrors: [{ sourceFile, rowNumber: 1, orderNumber: "", error: "File is empty" }], columnCount: 0, columnNames: [], previewRows: [] };
  }
  ...
  if (missingColumns.length || syntaxErrors.length) return { records: [], fileErrors, columnCount: headers.length, columnNames, previewRows };
  ...
  return { records, fileErrors, columnCount: headers.length, columnNames, previewRows };
}
...
async function parseWorkbookFile(file) {
  try {
    ...
    return { records: [], fileErrors: [{ sourceFile: file.name, rowNumber: 1, orderNumber: "", error: `Unable to read spreadsheet: ${error.message}` }], columnCount: 0, columnNames: [], previewRows: [] };
  }
}
...
async function parsePdfFile(file) {
  try {
    ...
    return { records: [], fileErrors: [{ sourceFile: file.name, rowNumber: 1, orderNumber: "", error: `Unable to extract a sales table from PDF: ${error.message}` }], columnCount: 0, columnNames: [], previewRows: [] };
  }
}
